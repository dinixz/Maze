...
import sys
import math
from game import possible_moves_player, get_hint
...
botao_hint = pygame.Rect(770, 200, 50, 20)
...
while running:
    ...
    if playing:
        ...
        final_maze = greedy_search(maze, h2)
        if final_maze is None: #se não houver solução a partir do estado atual acaba o jogo TELA GAME OVER
            running = False
    ...
    for evento in pygame.event.get():
        ...
        #Durante Jogo
            ...
            if evento.type == pygame.KEYDOWN:
                moves = possible_moves_player(maze)
                if evento.key == pygame.K_w and moves.get('up'):
                    maze = moves.get('up')
                    
                elif evento.key == pygame.K_s and moves.get('down'):
                    maze = moves.get('down')

                elif evento.key == pygame.K_a and moves.get('left'):
                    maze = moves.get('left')

                elif evento.key == pygame.K_d and moves.get('right'):
                    maze = moves.get('right')
                
                desenhar_labirinto(maze)
            
            elif evento.type == pygame.MOUSEBUTTONDOWN:
                if botao_hint.collidepoint(evento.pos):
